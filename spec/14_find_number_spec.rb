...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_min5_max9) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        expect(game_min5_max9.make_guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_min8_max9) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        expect(game_min8_max9.make_guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game_min0_max3) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        expect(game_min0_max3.make_guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_min3_max3) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        expect(game_min3_max3.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    let(:answer) { double('random_number', value: 8) }
    context 'when guess and random_number are equal' do
      ...
      subject(:game_over) { described_class.new(0, 9, answer, 8) }
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_not_over) { described_class.new(0, 9, answer, 10) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:guess_avg_game) { described_class.new(5, 8, number_range, 7) }

      before do
        guess_avg_game.update_range
      end

      it 'updates min to the same value as max' do
        expect(guess_avg_game.min).to eq(guess_avg_game.max)
      end

      it 'does not update max' do
        expect(guess_avg_game.min).to eq(8)
      end
    end
  end
end
