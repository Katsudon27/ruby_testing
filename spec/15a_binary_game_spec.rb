...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        invalid_input_one = '11'
        invalid_input_two = 'ab'
        valid_input = '4'
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        min = game_input.instance_variable_get(:@minimum)
        max = game_input.instance_variable_get(:@maximum)
        expect(game_input.verify_input(min, max, valid_input.to_i)).to eq(valid_input.to_i)
      end
    end
  ...
  describe '#display_binary_search' do
    ...
    context 'when game_over? is false five times' do
      ...
      it 'calls display_turn_order five times' do
        expect(game_display).to receive(:display_turn_order).exactly(5).with(search_display)
        game_display.display_binary_search(search_display)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    let(:search_guess) { instance_double(BinarySearch) }

    before do
      # You'll need to create a few method stubs.
      allow(search_guess).to receive(:make_guess)
      allow(game_turn_order).to receive(:display_guess)
      allow(search_guess).to receive(:update_range)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { game_turn_order.display_turn_order(search_guess) }.to change { game_turn_order.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(search_guess).to receive(:make_guess).once
      game_turn_order.display_turn_order(search_guess)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(search_guess).to receive(:update_range).once
      game_turn_order.display_turn_order(search_guess)
    end
