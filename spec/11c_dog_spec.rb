...
describe Dog do
  ...
  subject(:my_dog) { described_class.new(name: 'Henry', breed: 'Golden Retriever', color: 'brown') }

  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when Dog talks' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(subject).not_to be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(subject).to be_sleeping
  end
end
